 ...
 */
string format_html_add_tag(const string &s, const string &tag, const string &attr) {
...
void reporter_html::end() {
  report << "<br>\n";
  report << "<div class=\"topcorner\"> Failed checks: " + to_string(err_count) + "</div>\n";
  report << "</html>\n";
}
...
void reporter_html::add_row(const node_info_t& inf) {
  ...
  if (!inf.generator.empty()) {
    report << add_tooltip("From file ./" + inf.generator + ", line " + to_string(inf.generator_line));
...
string reporter_log::assemble_info(const node_info_t& inf) {
  ...
  result += " was hit " + to_string(inf.hit_count) + " times!";
...
void reporter_log::format_fail(const node_info_t& inf) {

  this->err_count++;
  report << "*CL_ERR in " << assemble_info(inf) << "\n";
